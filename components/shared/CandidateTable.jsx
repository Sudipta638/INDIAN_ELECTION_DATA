...
export const columns = [
    {
      ...
      cell: ({ row }) => (
        <div className="text-gray-700 font-semibold pl-6 w-20">
          {row.getValue("seatname")}
        </div>
      ),
    },
    {
    ...
    },
    {
        ...
        header: ({ column }) => {
            ...
            <Button
                variant="ghost"
                className="capitalize text-center ml-16  text-lg"
                onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
            >
                Party Name
                <CaretSortIcon className="ml-2 h-4 w-2" />
  ...
  export function DataTableDemo({ data }) {
    ...
    return (
      <div className="w-full">
        <div className="w-full">
          <Input
            placeholder="Filter Seat Names..."
            value={table.getColumn("seatname")?.getFilterValue() ?? ""}
            onChange={(event) =>
              table.getColumn("seatname")?.setFilterValue(event.target.value)
            }
...
const Tables = (
    ...
    const fetchCandidateDetails = async () => {
        const acResponse = await fetch(`http://localhost:8000/api/candidatedetailsonac/${cand_name}`);
        const acData = await acResponse.json();
        const pcResponse = await fetch(`http://localhost:8000/api/candidatedetailsonpc/${cand_name}`);
